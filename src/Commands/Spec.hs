...
createSpecFile cabalPath genPkgDesc flags = do
    ...
    let putHdr hdr val = hPutStrLn h (hdr ++ ":" ++ padding hdr ++ val)
        ...
        year = formatTime defaultTimeLocale "%Y" now
    ...
    putHdr "Name" (if isExec then (if isLib then name else pkgname) else pkgname)
    putHdr "Version" version
    putHdr "Release" release
